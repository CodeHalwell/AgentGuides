...
|---------|------|---------|
| 2.7.1 | August 24, 2026 | **New source-verified examples for 10 sampled classes.** Added dedicated example sections (each cross-checked via live introspection against `google-adk==2.7.1`) covering: `BuiltInCodeExecutor` (full signature + `is_gemini_model` model check + `GOOGLE_ADK_IGNORE_MODEL_ID_CHECK`), `VertexAiCodeExecutor` (`resource_name` reuse + output file `mime_type` handling), `TransferToAgentTool` (manual enum-constrained construction), `LoadMemoryTool` (JSON schema branch + implicit instruction injection), `RunConfig` (every field in one example + `custom_metadata` tracing + `get_session_config` window), `ContextCacheConfig` (`create_http_options` timeout + model-minimum floor rules), `LoggingPlugin` (subclass-for-JSON-logs + callback prefix table), `SaveFilesAsArtifactsPlugin` (`attach_file_reference=False` + `user:` prefix + 20 MB ceiling + pending-delta lifecycle), `AgentEvaluator.evaluate_eval_set` (`output_file` CSV + `artifact_service` preload + `eval_set_results_manager`), `Workflow` (`max_concurrency` semantics + `state_schema` compile-time validation). |
