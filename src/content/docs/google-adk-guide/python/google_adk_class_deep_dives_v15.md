...
| **[16](./google_adk_class_deep_dives_v16/)** | **`AgentCardBuilder`/`to_a2a`, part converters, `AdkEventToA2AEventsConverter`, `AuthScheme`/`OAuthGrantType`/`ExtendedOAuth2`, `McpInstructionProvider`, `BaseArtifactService`/`ArtifactVersion`/`InMemoryArtifactService`, `RequestInput`/HITL utils, `RetryConfig`+internals, `TaskResultAggregator`/`ExecutorContext`, `ToolConfig`/`BaseToolConfig`** |
