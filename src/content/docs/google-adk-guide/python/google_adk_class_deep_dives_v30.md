...
description: "Source-verified deep dives into 10 more google-adk 2.3.0 classes: GCPSkillRegistry (Vertex AI skill registry; lazy AsyncClient init; get_skill downloads zipped filesystem; search_skills delegates to _client.skills.retrieve; env vars GOOGLE_CLOUD_PROJECT/LOCATION), ApiRegistry (Cloud API Registry MCP toolset bridge; paginated server list on init; filter enabled=false; get_toolset builds McpToolset; header_provider per-request injection), AgentRegistry+AgentRegistrySingleMcpToolset (Agent Registry v1alpha; auto-IAM auth via GcpAuthProviderScheme; get_mcp_toolset/list_endpoints/get_endpoint/get_model_name; OTel tracing metadata injection), EnterpriseWebSearchTool singleton (enterprise_web_search built-in tool; process_llm_request injects types.EnterpriseWebSearch; Gemini 2+ only; ValueError on Gemini 1 with other tools), MultiTurnTaskSuccessV1Evaluator (multi-turn LLM-as-judge rubric; delegates to _MultiTurnVertexiAiEvalFacade; score [0,1]; requires GOOGLE_CLOUD_PROJECT+LOCATION), upgrade() DB migration runner (MIGRATIONS dict; SCHEMA_VERSION_0_PICKLE→1_JSON; multi-step uses temp SQLite; no in-place migration; auto-cleanup temp files), generate_content_via_interactions (Interactions API async generator; previous_interaction_id optimisation; converts LlmRequest→steps+tools+system_instruction; stream/non-stream; tracks current_interaction_id), _BasicLlmRequestProcessor+_build_basic_request (sets model from canonical_model; deep-copies generate_content_config; output_schema guarded by can_use_output_schema_with_tools; all live_connect_config fields), SandboxClient CDP commands (Vertex AI Computer Use; make_cdp_request/make_cdp_batch_request; batch /cdps endpoint with sequential fallback; get_screenshot with retries; _META_KEY_MAP+_MODIFIER_MAP), RubricBasedFinalResponseQualityV1Evaluator (rubric-based final response quality; @experimental; RUBRIC_TYPE=FINAL_RESPONSE_QUALITY; 10240-token silent thinking; verdict yes/no→1.0/0.0; DefaultAutoRaterResponseParser)."
...
`RubricBasedFinalResponseQualityV1Evaluator` extends `RubricBasedEvaluator` and uses an LLM (via the Vertex AI Gen AI Eval SDK) to judge the quality of an agent's final response against user-defined rubrics. It is gated behind `@experimental`. The internal prompt instructs the LLM to think silently with a 10 240-token budget and produce `yes`/`no` verdicts that map to `1.0`/`0.0`.
