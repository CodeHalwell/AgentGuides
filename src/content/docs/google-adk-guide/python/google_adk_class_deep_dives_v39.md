...
**Nested workflow detection:** `_ENTRYPOINT_WORKFLOW_KEY` is an OpenTelemetry context key (created via `context_api.create_key()`) set to `True` once the first workflow span opens. Subsequent workflow spans check this key and emit `gen_ai.workflow.nested = True` if already set. The root workflow never emits this attribute.
...
`Graph.get_next_pending_nodes()` is the routing engine: given the current node name and the route(s) it emitted, it returns the list of successor node names to move to `PENDING` state. `DEFAULT_ROUTE = "__DEFAULT__"` is an **edge-side tag**: when you define an edge with `route=DEFAULT_ROUTE`, that edge fires as a fallback only when no specific-route edge was matched. Nodes may also explicitly emit `ctx.route = DEFAULT_ROUTE` — because `"__DEFAULT__"` does not match any specific edge route in `get_next_pending_nodes`, `matched_specific_route` stays `False` and the fallback edge fires exactly as intended. This is the same net effect as emitting no route at all, and the pattern is used in other ADK guide examples when you want to make the "fallback" branch explicit rather than relying on a silent no-emit.
