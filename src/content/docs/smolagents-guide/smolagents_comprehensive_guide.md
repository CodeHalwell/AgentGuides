...
Latest: 1.25.0 | Updated: May 18, 2026
...
|---------|------|---------|
| 1.25.0 | 2026-05-18 | Version bumped 1.24.0 → 1.25.0; `Latest:` header and date updated. New `WasmExecutor` backend noted. Core symbols (`CodeAgent`, `ToolCallingAgent`, `tool`, `Tool`, `InferenceClientModel`, `LiteLLMModel`, `DuckDuckGoSearchTool`, `WikipediaSearchTool`, `VisitWebpageTool`, `MultiStepAgent`) verified against installed `smolagents==1.25.0` (`.routine-envs/check-0518-py`); no `DeprecationWarning` emissions. | Claude routine |
