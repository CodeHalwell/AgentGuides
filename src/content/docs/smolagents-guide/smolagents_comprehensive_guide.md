...
|---------|------|---------|
| 1.25.0 | 2026-05-15 | Version bumped 1.24.0 → 1.25.0; `Latest:` header updated; revision history entry added. `InferenceClientModel` default `model_id` changed to `Qwen/Qwen3-Next-80B-A3B-Thinking`. Core imports (`CodeAgent`, `InferenceClientModel`, `ToolCallingAgent`, `tool`, `Tool`) verified against installed `smolagents==1.25.0` (`.routine-envs/check-0515-smol`); no `DeprecationWarning` emissions. |
