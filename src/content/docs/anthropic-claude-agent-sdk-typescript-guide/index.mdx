...
## What's new in v0.2.141 (May 2026)

- **v0.2.141 (May 14)** — Patch release; stability and dependency updates. Full export surface verified against installed `@anthropic-ai/claude-agent-sdk@0.2.141` (`.routine-envs/check-0514-node`): `query`, `tool`, `createSdkMcpServer`, session management APIs (`forkSession`, `listSessions`, `getSessionInfo`, `getSessionMessages`, `InMemorySessionStore`, etc.), error types (`AbortError`, `DirectConnectError`), transport (`DirectConnectTransport`), and constants (`EXIT_REASONS`, `HOOK_EVENTS`, `SYSTEM_PROMPT_DYNAMIC_BOUNDARY`) all present.
