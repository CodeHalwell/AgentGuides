...
## What's new in v0.3.143 (May 2026)

- **v0.3.143 (May 18)** — Minor series bump (0.2.x → 0.3.x). The three `unstable_v2_*` exports (`unstable_v2_createSession`, `unstable_v2_prompt`, `unstable_v2_resumeSession`) have been removed from the public API surface. All 25 remaining exports verified against installed `@anthropic-ai/claude-agent-sdk@0.3.143` (`.routine-envs/check-0518-node`): `query`, `tool`, `createSdkMcpServer`, session helpers (`forkSession`, `listSessions`, `getSessionInfo`, `getSessionMessages`, `getSubagentMessages`, `importSessionToStore`, `listSubagents`, `deleteSession`, `renameSession`, `tagSession`, `InMemorySessionStore`, `resolveSettings`, `startup`, `filterEscalatingDefaultMode`, `foldSessionSummary`), error types (`AbortError`, `DirectConnectError`), transport (`DirectConnectTransport`, `parseDirectConnectUrl`), and constants (`EXIT_REASONS`, `HOOK_EVENTS`, `SYSTEM_PROMPT_DYNAMIC_BOUNDARY`) all present.
- **v0.2.141 (May 14)** — Patch release; stability and dependency updates. Full 28-export surface verified against installed `@anthropic-ai/claude-agent-sdk@0.2.141` (`.routine-envs/check-0514-node`).
...
There is no `Agent` class in `@anthropic-ai/claude-agent-sdk` 0.3.143. The public entry points are `query()` (one-shot) and the lower-level exports listed on the package. Earlier drafts of this page documented a `new Agent({...})` constructor — it does not exist.
...
|------|-------------------------------|--------------------|----------|
| 2026-05-18 | @anthropic-ai/claude-agent-sdk 0.3.143 | Version bumped 0.2.141 → 0.3.143 (minor series bump 0.2.x → 0.3.x); "What's new" updated; install note updated; comprehensive guide header updated; revision history entry added. `unstable_v2_createSession`, `unstable_v2_prompt`, `unstable_v2_resumeSession` confirmed removed. All 25 remaining exports verified against installed `@anthropic-ai/claude-agent-sdk@0.3.143` (`.routine-envs/check-0518-node`). | Claude routine |
