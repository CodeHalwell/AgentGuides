...
- **v0.3.142 (May 15)** — Minor release (0.2.x → 0.3.x series). The three `unstable_v2_*` exports present in 0.2.x (`unstable_v2_createSession`, `unstable_v2_prompt`, `unstable_v2_resumeSession`) have been removed. Do not reference these in any guide or production code. Stable export surface is now 25 exports — all verified against installed `@anthropic-ai/claude-agent-sdk@0.3.142` (`.routine-envs/check-0515-node`): `query`, `tool`, `createSdkMcpServer`, session helpers (`forkSession`, `listSessions`, `getSessionInfo`, `getSessionMessages`, `getSubagentMessages`, `importSessionToStore`, `InMemorySessionStore`, `listSubagents`, `renameSession`, `tagSession`, `deleteSession`, `foldSessionSummary`, `filterEscalatingDefaultMode`), transport (`DirectConnectTransport`, `parseDirectConnectUrl`), utilities (`startup`, `resolveSettings`), error types (`AbortError`, `DirectConnectError`), and constants (`EXIT_REASONS`, `HOOK_EVENTS`, `SYSTEM_PROMPT_DYNAMIC_BOUNDARY`).
