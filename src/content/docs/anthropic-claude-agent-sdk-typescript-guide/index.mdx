...
## What's new in v0.3.142 (May 2026)

- **v0.3.142 (May 15)** — Minor release (0.2.x → 0.3.x series). The three `unstable_v2_*` exports present in 0.2.x (`unstable_v2_createSession`, `unstable_v2_prompt`, `unstable_v2_resumeSession`) have been removed. Do not reference these in any guide or production code. Stable export surface is now **25 exports** — all verified against installed `@anthropic-ai/claude-agent-sdk@0.3.142` (`.routine-envs/check-0515-node`):
  - **Entry points:** `query`, `tool`, `createSdkMcpServer`
  - **Session helpers:** `forkSession`, `listSessions`, `getSessionInfo`, `getSessionMessages`, `getSubagentMessages`, `importSessionToStore`, `InMemorySessionStore`, `listSubagents`, `renameSession`, `tagSession`, `deleteSession`, `foldSessionSummary`, `filterEscalatingDefaultMode`
  - **Transport:** `DirectConnectTransport`, `parseDirectConnectUrl`
  - **Utilities:** `startup`, `resolveSettings`
  - **Error types:** `AbortError`, `DirectConnectError`
  - **Constants:** `EXIT_REASONS`, `HOOK_EVENTS`, `SYSTEM_PROMPT_DYNAMIC_BOUNDARY`
- **v0.2.141 (May 14)** — Patch release; stability and dependency updates. Full export surface verified against installed `@anthropic-ai/claude-agent-sdk@0.2.141` (`.routine-envs/check-0514-node`): 28 exports confirmed including `query`, `tool`, `createSdkMcpServer`, session helpers, error types, transport, and constants.
- **v0.2.140 (May 13)** — Patch release; stability and dependency updates. Runtime exports (`query`, `tool`, `createSdkMcpServer`) verified against installed `@anthropic-ai/claude-agent-sdk@0.2.140` (`.routine-envs/check-0513-node`).
...
There is no `Agent` class in `@anthropic-ai/claude-agent-sdk`. The public entry points are `query()` (one-shot) and the lower-level exports listed on the package. Earlier drafts of this page documented a `new Agent({...})` constructor — it does not exist.
...
|------|-------------------------------|--------------------|----------|
| 2026-05-15 | @anthropic-ai/claude-agent-sdk 0.3.142 | Version bumped 0.2.141 → 0.3.142 (0.2.x → 0.3.x series); "What's new" updated noting removal of `unstable_v2_createSession`, `unstable_v2_prompt`, `unstable_v2_resumeSession`; comprehensive guide header updated; revision history entry added. All 25 stable exports verified against installed `@anthropic-ai/claude-agent-sdk@0.3.142` (`.routine-envs/check-0515-node`): PASS. | Claude routine |
