...
## What's new in v1.97.0 (May 2026)

- **v1.97.0 (May 18)** — Minor release. `Embedder` and `EmbeddingModel` (with `EmbeddingResult`, `EmbeddingSettings`, `KnownEmbeddingModelName`) now exported directly from the top-level `pydantic_ai` namespace — no need to import from `pydantic_ai.embeddings`. All guide-referenced symbols (`Agent`, `RunContext`, `ModelRetry`, `AgentRunResult`, `StreamedRunResult`, `UsageLimits`, `RunUsage`, `capture_run_messages`, `AgentRunResultEvent`, `AgentEventStream`, `ConcurrencyLimiter`, `limit_model_concurrency`) verified against installed `pydantic-ai==1.97.0` (`.routine-envs/check-0518-py`); no `DeprecationWarning` emissions.
...
|------|-------------------------------|--------------------|----------|
| 2026-05-18 | pydantic-ai 1.97.0 | Version bumped 1.96.0 → 1.97.0; "What's new" updated noting top-level `Embedder`/`EmbeddingModel` exports; comprehensive guide header updated; revision history entry added. All guide-referenced symbols verified against installed `pydantic-ai==1.97.0` (`.routine-envs/check-0518-py`); no `DeprecationWarning` emissions. | Claude routine |
