...
## What's new in v1.96.1 (May 2026)

- **v1.96.1 (May 15)** — Patch release; stability and dependency updates. All guide-referenced symbols (`Agent`, `RunContext`, `ModelRetry`, `AgentRunResult`, `StreamedRunResult`, `UsageLimits`, `RunUsage`, `capture_run_messages`, `ConcurrencyLimiter`, `limit_model_concurrency`) verified against installed `pydantic-ai==1.96.1` (`.routine-envs/check-0515-py`); no `DeprecationWarning` emissions.
- **v1.96.0 (May 14)** — Minor release. New concurrency management API: `ConcurrencyLimiter(max_running, max_queued=None)` and `limit_model_concurrency(model, limiter)` — wraps any `Model` with a concurrency gate to cap parallel model requests. All guide-referenced symbols (`Agent`, `RunContext`, `ModelRetry`, `AgentRunResult`, `StreamedRunResult`, `UsageLimits`, `RunUsage`, `capture_run_messages`, `AgentRunResultEvent`, `AgentEventStream`) verified against installed `pydantic-ai==1.96.0` (`.routine-envs/check-0514-py`); no `DeprecationWarning` emissions.
...
|------|-------------------------------|--------------------|----------|
| 2026-05-15 | pydantic-ai 1.96.1 | Version bumped 1.96.0 → 1.96.1; "What's new" updated with patch release note; comprehensive guide header updated; revision history entry added. All guide-referenced symbols verified against installed `pydantic-ai==1.96.1` (`.routine-envs/check-0515-py`); no `DeprecationWarning` emissions. | Claude routine |
