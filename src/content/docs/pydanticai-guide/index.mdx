...
## What's new in v1.99.0 (May 2026)
...
| 2026-05-20 | pydantic-ai 1.99.0 | Library installed and source inspected directly. Ten classes deep-dived from source: `Agent`, `RunContext`, `UsageLimits`, `ConcurrencyLimiter`, `FunctionToolset`, `AgentRunResult`, `ToolDefinition`, `ModelRetry`, `TestModel`, `FunctionModel`. Class deep-dives guide (`pydantic_ai_class_deep_dives.md`) expanded with `UsageLimits` (§11) and `ConcurrencyLimiter` (§12). `pydantic_ai_advanced_error_testing.md` fully rewritten (was 32 lines stub). `pydantic_ai_streaming_server_fastapi.md` fully rewritten with correct pydantic-ai streaming APIs. All `Verified against` headers updated to 1.99.0. | Claude |
