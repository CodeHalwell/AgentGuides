...
| `uuid7()` + `PydanticAIDeprecationWarning` | `_uuid` / `_warnings` | RFC 9562 Method 1, 42-bit counter, global lock; `UserWarning` subclass (visible by default), not `DeprecationWarning` |
...
| `Toolset` + `WrapperCapability` | `capabilities/toolset` / `capabilities/wrapper` | minimal single-field bridge; `__post_init__` inherits `id`/`defer_loading`, `apply()` visitor, `for_run` `replace()` pattern |
...
| `SetToolMetadata` | `capabilities/set_tool_metadata` | `**metadata` kwargs, shallow top-level dict merge (`{**old, **new}`), spec-serializable |
