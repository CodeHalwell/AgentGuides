...
- Guard calls to `clear_cache()` / `aclear_cache()` with `if compiled.cache is not None:` before calling — calling on a graph compiled without `cache=` raises `ValueError`. TTL-expired entries disappear silently on next read — they do not cause an error.
