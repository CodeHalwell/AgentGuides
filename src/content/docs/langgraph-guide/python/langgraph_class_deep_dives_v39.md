...
- The `response_format` kwarg activates a structured-output node; when set, the compiled graph adds `"final_response"` after the agent decides to stop, and the output lands in `state["structured_response"]`.
