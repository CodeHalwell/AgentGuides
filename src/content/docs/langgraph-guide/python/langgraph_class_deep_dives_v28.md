...
- **`list()` ordering** — checkpoints are listed newest-first by sorting the inner dict by checkpoint_id lexicographically in descending order (`sorted(..., key=lambda x: x[0], reverse=True)`). Since checkpoint IDs are UUIDv6 (`uuid6(clock_seq=step)`), which are time-ordered and monotonically increasing, lexicographic descending order reliably yields newest-first.
