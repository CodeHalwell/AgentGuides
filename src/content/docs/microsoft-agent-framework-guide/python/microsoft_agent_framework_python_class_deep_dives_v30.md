...
HITL: after `run()` raises `WorkflowInterrupted`, `agent.pending_requests` contains the suspended `WorkflowEvent` objects. Resume by passing `responses` keyed by `request_id`:
