...
description: "Agent, chat, and function middleware in agent-framework-core 1.2.1 — real signatures, short-circuit patterns, telemetry, retries, and redaction."
...
All three ship in `agent_framework`; imports below are stable in `agent-framework-core==1.2.1`.
...
The `context_providers` parameter on `Agent` is still **plural** in 1.2.1, contrary to earlier drafts of this guide.
