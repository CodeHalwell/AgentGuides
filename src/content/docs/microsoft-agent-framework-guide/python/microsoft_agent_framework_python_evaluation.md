...
description: "Quality gates for agents and workflows: evaluate_agent, evaluate_workflow, LocalEvaluator, @evaluator decorator, keyword_check and tool_call helpers — all from agent-framework-core 1.2.1."
...
Verified against `agent-framework-core==1.2.1` (`agent_framework._evaluation`). This module is marked `experimental` — API may evolve.
