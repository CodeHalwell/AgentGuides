...
description: "Real imports and constructors for every first-party chat client in agent-framework 1.2.1: OpenAI, Azure OpenAI, Microsoft Foundry, Foundry Local, Anthropic, Ollama, Bedrock, GitHub Copilot, Copilot Studio."
...
This page was verified against `agent-framework-core==1.2.1` and provider packages at `1.0.0b260428` (April 2026). Each sub-package is imported lazily from the `agent_framework.<provider>` namespace — you install the provider package and import from `agent_framework.<provider>`.
...
async with AzureCliCredential() as credential:
    ...
    response = await agent.run("Summarise agent-framework 1.2.1 in one line.")
```
