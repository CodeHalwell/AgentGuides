...
Which agent should handle this? Reply with ONLY the agent name or DONE."""
...
def route_to_agent(state: SupervisorState) -> str:
    ...
    choice = state.get("next_agent", "").lower()
...
The Reflection-Action pattern for autonomous agents:

```python
from langgraph.prebuilt import create_react_agent

# Using the high-level factory
model = ChatAnthropic(model="claude-3-5-sonnet-20241022")
...
react_agent = create_react_agent(
    ...
    tools=tools,
    checkpointer=InMemorySaver(),
    prompt="You are a helpful research assistant. Think before acting."
)

# Use it - automatically handles ReAct loop
result = react_agent.invoke({
    "messages": [{
        "role": "user",
        "content": "Research population of Tokyo and calculate 15% of that"
    }]
})
