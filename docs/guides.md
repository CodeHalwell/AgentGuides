---
layout: default
title: "All Guides"
description: "Complete list of all agent framework guides - Updated November 2025"
---

# All Framework Guides

**Complete Directory of All AI Agent Framework Documentation**

**Last Updated:** November 17, 2025
**Frameworks:** 18
**Languages:** Python, TypeScript, .NET, Go
**Total Files:** 160+

---

## 📖 How to Use This Page

Each framework section includes links to:
- **README** - Quick start and overview
- **GUIDE_INDEX** - Complete navigation (if available)
- **Comprehensive** - Full technical reference
- **Production** - Deployment and scaling
- **Diagrams** - Architecture visualizations
- **Recipes** - Code examples
- **Special Guides** - Framework-specific advanced topics

---

## 🎯 Multi-Language Frameworks

### Google ADK
**Languages:** Python, Go
**Latest:** Python v1.18.0, Go v0.1.0 (November 2025)

#### Python
- [📂 Python Directory](../Google_ADK_Guide/python/)
- [📖 README](../Google_ADK_Guide/python/README.md)
- [🗺️ GUIDE_INDEX](../Google_ADK_Guide/python/GUIDE_INDEX)
- [📚 Comprehensive](../Google_ADK_Guide/python/google_adk_comprehensive_guide)
- [🚀 Production](../Google_ADK_Guide/python/google_adk_production_guide)
- [🎨 Diagrams](../Google_ADK_Guide/python/google_adk_diagrams)
- [👨‍💻 Recipes](../Google_ADK_Guide/python/google_adk_recipes)
- [⚡ Advanced Python](../Google_ADK_Guide/python/google_adk_advanced_python)
- [🔐 IAM Examples](../Google_ADK_Guide/python/google_adk_iam_examples)

#### Go
- [📂 Go Directory](../Google_ADK_Guide/go/)
- [📖 README](../Google_ADK_Guide/go/README.md)
- [🗺️ GUIDE_INDEX](../Google_ADK_Guide/go/GUIDE_INDEX)
- [📚 Comprehensive](../Google_ADK_Guide/go/google_adk_go_comprehensive_guide)
- [🚀 Production](../Google_ADK_Guide/go/google_adk_go_production_guide)
- [👨‍💻 Recipes](../Google_ADK_Guide/go/google_adk_go_recipes)

---

### Semantic Kernel
**Languages:** Python, .NET
**Latest:** v1.38.0

#### Python
- [📂 Python Directory](../Semantic_Kernel_Guide/python/)
- [📖 README](../Semantic_Kernel_Guide/python/README.md)
- [🗺️ GUIDE_INDEX](../Semantic_Kernel_Guide/python/GUIDE_INDEX)
- [📚 Comprehensive](../Semantic_Kernel_Guide/python/semantic_kernel_comprehensive_python)
- [🚀 Production](../Semantic_Kernel_Guide/python/semantic_kernel_production_python)
- [🎨 Diagrams](../Semantic_Kernel_Guide/python/semantic_kernel_diagrams_python)
- [👨‍💻 Recipes](../Semantic_Kernel_Guide/python/semantic_kernel_recipes_python)
- [⚡ Advanced Multi-Agent](../Semantic_Kernel_Guide/python/semantic_kernel_advanced_multi_agent_python)
- [🔧 Middleware](../Semantic_Kernel_Guide/python/semantic_kernel_middleware_python)
- [🌊 Streaming Server](../Semantic_Kernel_Guide/python/semantic_kernel_streaming_server_fastapi)

#### .NET
- [📂 .NET Directory](../Semantic_Kernel_Guide/dotnet/)
- [📖 README](../Semantic_Kernel_Guide/dotnet/README.md)
- [🗺️ GUIDE_INDEX](../Semantic_Kernel_Guide/dotnet/GUIDE_INDEX)
- [📚 Comprehensive](../Semantic_Kernel_Guide/dotnet/semantic_kernel_comprehensive_dotnet)
- [🚀 Production](../Semantic_Kernel_Guide/dotnet/semantic_kernel_production_dotnet)
- [🎨 Diagrams](../Semantic_Kernel_Guide/dotnet/semantic_kernel_diagrams_dotnet)
- [👨‍💻 Recipes](../Semantic_Kernel_Guide/dotnet/semantic_kernel_recipes_dotnet)

---

### LlamaIndex
**Languages:** Python, TypeScript
**Latest:** Python v0.14.8, Workflows 1.0

#### Python
- [📂 Python Directory](../LlamaIndex_Guide/python/)
- [📖 README](../LlamaIndex_Guide/python/README.md)
- [🗺️ GUIDE_INDEX](../LlamaIndex_Guide/python/GUIDE_INDEX)
- [📚 Comprehensive](../LlamaIndex_Guide/python/llamaindex_comprehensive_guide)
- [🚀 Production](../LlamaIndex_Guide/python/llamaindex_production_guide)
- [🎨 Diagrams](../LlamaIndex_Guide/python/llamaindex_diagrams)
- [👨‍💻 Recipes](../LlamaIndex_Guide/python/llamaindex_recipes)
- [⚡ Advanced Implementations](../LlamaIndex_Guide/python/llamaindex_advanced_implementations)
- [📊 Observability](../LlamaIndex_Guide/python/llamaindex_observability_python)
- [🌊 Streaming Server](../LlamaIndex_Guide/python/llamaindex_streaming_server_fastapi)

#### TypeScript
- [📂 TypeScript Directory](../LlamaIndex_Guide/typescript/)
- [📖 README](../LlamaIndex_Guide/typescript/README.md)
- [🗺️ GUIDE_INDEX](../LlamaIndex_Guide/typescript/GUIDE_INDEX)
- [📚 Workflows Comprehensive](../LlamaIndex_Guide/typescript/llamaindex_workflows_typescript_comprehensive_guide)
- [🚀 Production](../LlamaIndex_Guide/typescript/llamaindex_typescript_production_guide)
- [👨‍💻 Recipes](../LlamaIndex_Guide/typescript/llamaindex_typescript_recipes)

---

### LangGraph
**Languages:** Python, TypeScript
**Latest:** Python v1.0.3 (Nov 10, 2025), TypeScript v1.0.2

#### Python
- [📂 Python Directory](../LangGraph_Guide/python/)
- [📖 README](../LangGraph_Guide/python/README.md)
- [📚 Comprehensive](../LangGraph_Guide/python/langgraph_comprehensive_guide)
- [🚀 Production](../LangGraph_Guide/python/langgraph_production_guide)
- [🎨 Diagrams](../LangGraph_Guide/python/langgraph_diagrams)
- [👨‍💻 Recipes](../LangGraph_Guide/python/langgraph_recipes)
- [⚡ Advanced Error Recovery](../LangGraph_Guide/python/langgraph_advanced_error_recovery)
- [⚙️ Advanced Implementations](../LangGraph_Guide/python/langgraph_advanced_implementations)
- [📊 Observability](../LangGraph_Guide/python/langgraph_observability_python)
- [🚀 Performance Optimization](../LangGraph_Guide/python/langgraph_performance_optimization)
- [🌊 Streaming Server (FastAPI)](../LangGraph_Guide/python/langgraph_streaming_server_fastapi)

#### TypeScript
- [📂 TypeScript Directory](../LangGraph_Guide/typescript/)
- [📖 README](../LangGraph_Guide/typescript/README.md)
- [🗺️ GUIDE_INDEX](../LangGraph_Guide/typescript/GUIDE_INDEX)
- [📚 Comprehensive](../LangGraph_Guide/typescript/langchain_langgraph_comprehensive_guide)
- [🚀 Production](../LangGraph_Guide/typescript/langchain_langgraph_production_guide)
- [🎨 Diagrams](../LangGraph_Guide/typescript/langchain_langgraph_diagrams)
- [👨‍💻 Recipes](../LangGraph_Guide/typescript/langchain_langgraph_recipes)
- [🔧 Middleware](../LangGraph_Guide/typescript/langgraph_middleware_typescript)
- [📊 Observability](../LangGraph_Guide/typescript/langgraph_observability_typescript)
- [🌊 Streaming Server (Express)](../LangGraph_Guide/typescript/langgraph_streaming_server_express)

---

### Microsoft Agent Framework
**Languages:** Python, .NET
**Latest:** Preview (October 2025)

#### Base Framework
- [📂 Framework Guide](../Microsoft_Agent_Framework_Guide/)
- [📖 README](../Microsoft_Agent_Framework_Guide/README.md)
- [📚 Comprehensive](../Microsoft_Agent_Framework_Guide/microsoft_agent_framework_comprehensive_guide)
- [🚀 Production](../Microsoft_Agent_Framework_Guide/microsoft_agent_framework_production_guide)
- [🎨 Diagrams](../Microsoft_Agent_Framework_Guide/microsoft_agent_framework_diagrams)
- [👨‍💻 Recipes](../Microsoft_Agent_Framework_Guide/microsoft_agent_framework_recipes)
- [🔗 A2A Protocol](../Microsoft_Agent_Framework_Guide/microsoft_agent_framework_a2a_protocol) **NEW 2025**
- [📊 Graphs & Declarative](../Microsoft_Agent_Framework_Guide/microsoft_agent_framework_graphs_declarative) **NEW 2025**
- [🏢 Enterprise 2025](../Microsoft_Agent_Framework_Guide/microsoft_agent_framework_enterprise_2025) **NEW 2025**
- [📋 Migration Notice](../Microsoft_Agent_Framework_Guide/MIGRATION_NOTICE)

#### Python
- [📂 Python Directory](../Microsoft_Agent_Framework_Guide/python/)
- [📖 README](../Microsoft_Agent_Framework_Guide/python/README.md)
- [🗺️ GUIDE_INDEX](../Microsoft_Agent_Framework_Guide/python/GUIDE_INDEX.md)
- [📚 Comprehensive](../Microsoft_Agent_Framework_Guide/python/microsoft_agent_framework_python_comprehensive_guide.md)
- [⚡ Advanced](../Microsoft_Agent_Framework_Guide/python/microsoft_agent_framework_python_advanced.md)
- [✨ 2025 Features](../Microsoft_Agent_Framework_Guide/python/microsoft_agent_framework_python_2025_features.md) **NEW**

#### .NET
- [📂 .NET Directory](../Microsoft_Agent_Framework_Guide/dotnet/)
- [📖 README](../Microsoft_Agent_Framework_Guide/dotnet/README.md)
- [🗺️ GUIDE_INDEX](../Microsoft_Agent_Framework_Guide/dotnet/GUIDE_INDEX.md)
- [📚 Comprehensive](../Microsoft_Agent_Framework_Guide/dotnet/microsoft_agent_framework_dotnet_comprehensive_guide.md)
- [🚀 Production](../Microsoft_Agent_Framework_Guide/dotnet/microsoft_agent_framework_dotnet_production_guide.md)
- [🎨 Diagrams](../Microsoft_Agent_Framework_Guide/dotnet/microsoft_agent_framework_dotnet_diagrams.md)
- [👨‍💻 Recipes](../Microsoft_Agent_Framework_Guide/dotnet/microsoft_agent_framework_dotnet_recipes.md)
- [⚡ Advanced](../Microsoft_Agent_Framework_Guide/dotnet/microsoft_agent_framework_dotnet_advanced.md)
- [✨ 2025 Features](../Microsoft_Agent_Framework_Guide/dotnet/microsoft_agent_framework_dotnet_2025_features.md) **NEW**
- [🔐 Azure RBAC](../Microsoft_Agent_Framework_Guide/dotnet/microsoft_agent_framework_azure_rbac.md)
- [🌊 Streaming Server](../Microsoft_Agent_Framework_Guide/dotnet/microsoft_agent_streaming_server_dotnet.md)

---

### OpenAI Agents SDK
**Languages:** Python, TypeScript
**Latest:** Production (Swarm replacement)

#### Python
- [📂 Python Directory](../OpenAI_Agents_SDK_Guides/)
- [📖 README](../OpenAI_Agents_SDK_Guides/README.md)
- [🗺️ GUIDE_INDEX](../OpenAI_Agents_SDK_Guides/GUIDE_INDEX)
- [📚 Comprehensive](../OpenAI_Agents_SDK_Guides/openai_agents_sdk_comprehensive_guide)
- [🚀 Production](../OpenAI_Agents_SDK_Guides/openai_agents_sdk_production_guide)
- [🎨 Diagrams](../OpenAI_Agents_SDK_Guides/openai_agents_sdk_diagrams)
- [👨‍💻 Recipes](../OpenAI_Agents_SDK_Guides/openai_agents_sdk_recipes)
- [⚡ Advanced Multi-Agent](../OpenAI_Agents_SDK_Guides/openai_agents_sdk_advanced_multi_agent)
- [🔧 Middleware](../OpenAI_Agents_SDK_Guides/openai_agents_sdk_middleware_python)
- [🌊 Streaming Server](../OpenAI_Agents_SDK_Guides/openai_streaming_server_fastapi)
- [✨ 2025 Features](../OpenAI_Agents_SDK_Guides/openai_agents_sdk_2025_features) **NEW**
- [🔄 Swarm Migration Guide](../OpenAI_Agents_SDK_Guides/openai_agents_sdk_swarm_migration_guide) **NEW**

#### TypeScript
- [📂 TypeScript Directory](../OpenAI_Agents_SDK_TypeScript_Guide/)
- [📖 README](../OpenAI_Agents_SDK_TypeScript_Guide/README.md)
- [🗺️ GUIDE_INDEX](../OpenAI_Agents_SDK_TypeScript_Guide/GUIDE_INDEX)
- [📚 Comprehensive](../OpenAI_Agents_SDK_TypeScript_Guide/openai_agents_sdk_typescript_comprehensive_guide)
- [🚀 Production](../OpenAI_Agents_SDK_TypeScript_Guide/openai_agents_sdk_typescript_production_guide)
- [🎨 Diagrams](../OpenAI_Agents_SDK_TypeScript_Guide/openai_agents_sdk_typescript_diagrams)
- [👨‍💻 Recipes](../OpenAI_Agents_SDK_TypeScript_Guide/openai_agents_sdk_typescript_recipes)
- [🔧 Middleware](../OpenAI_Agents_SDK_TypeScript_Guide/openai_agents_sdk_middleware_typescript)
- [✨ 2025 Features](../OpenAI_Agents_SDK_TypeScript_Guide/openai_agents_sdk_typescript_2025_features) **NEW**
- [🔄 Swarm Migration Guide](../OpenAI_Agents_SDK_TypeScript_Guide/openai_agents_sdk_typescript_swarm_migration_guide) **NEW**

---

### Anthropic Claude Agent SDK
**Languages:** Python, TypeScript
**Latest:** Python v0.72.1, TypeScript v0.68.0 (Rebranded from Code SDK)

#### Python
- [📂 Python Directory](../Anthropic_Claude_Agent_SDK_Guide/)
- [📖 README](../Anthropic_Claude_Agent_SDK_Guide/README.md)
- [📚 Comprehensive](../Anthropic_Claude_Agent_SDK_Guide/anthropic_claude_agent_sdk_comprehensive_guide)
- [🚀 Production](../Anthropic_Claude_Agent_SDK_Guide/anthropic_claude_agent_sdk_production_guide)
- [🎨 Diagrams](../Anthropic_Claude_Agent_SDK_Guide/anthropic_claude_agent_sdk_diagrams)
- [👨‍💻 Recipes](../Anthropic_Claude_Agent_SDK_Guide/anthropic_claude_agent_sdk_recipes)
- [🔄 Migration Guide](../Anthropic_Claude_Agent_SDK_Guide/anthropic_claude_agent_sdk_migration_guide) **NEW**

#### TypeScript
- [📂 TypeScript Directory](../Anthropic_Claude_Agent_SDK_TypeScript_Guide/)
- [📖 README](../Anthropic_Claude_Agent_SDK_TypeScript_Guide/README.md)
- [🗺️ GUIDE_INDEX](../Anthropic_Claude_Agent_SDK_TypeScript_Guide/GUIDE_INDEX)
- [📚 Comprehensive](../Anthropic_Claude_Agent_SDK_TypeScript_Guide/claude_agent_sdk_typescript_comprehensive_guide)
- [🚀 Production](../Anthropic_Claude_Agent_SDK_TypeScript_Guide/claude_agent_sdk_typescript_production_guide)
- [🎨 Diagrams](../Anthropic_Claude_Agent_SDK_TypeScript_Guide/claude_agent_sdk_typescript_diagrams)
- [👨‍💻 Recipes](../Anthropic_Claude_Agent_SDK_TypeScript_Guide/claude_agent_sdk_typescript_recipes)
- [⚡ Advanced Multi-Agent](../Anthropic_Claude_Agent_SDK_TypeScript_Guide/claude_agent_sdk_typescript_advanced_multi_agent)
- [🔧 Middleware](../Anthropic_Claude_Agent_SDK_TypeScript_Guide/claude_agent_sdk_typescript_middleware)
- [🔄 Migration Guide](../Anthropic_Claude_Agent_SDK_TypeScript_Guide/claude_agent_sdk_typescript_migration_guide) **NEW**

---

## 🐍 Python-Only Frameworks

### CrewAI
**Latest:** v1.4.1

- [📂 CrewAI Guide](../CrewAI_Guide/)
- [📖 README](../CrewAI_Guide/README.md)
- [🗺️ GUIDE_INDEX](../CrewAI_Guide/GUIDE_INDEX)
- [📚 Comprehensive](../CrewAI_Guide/crewai_comprehensive_guide)
- [🚀 Production](../CrewAI_Guide/crewai_production_guide)
- [🎨 Diagrams](../CrewAI_Guide/crewai_diagrams)
- [👨‍💻 Recipes](../CrewAI_Guide/crewai_recipes)
- [⚡ Advanced Multi-Agent](../CrewAI_Guide/crewai_advanced_multi_agent_python)
- [🌊 Flows Guide](../CrewAI_Guide/crewai_flows_guide) **NEW 2025**

---

### AutoGen (AG2)
**Latest:** v0.10.0 (Rebranded to AG2)

- [📂 AutoGen/AG2 Guide](../AutoGen_Guide/python/)
- [📖 README](../AutoGen_Guide/python/README.md)
- [📚 Comprehensive](../AutoGen_Guide/python/autogen_comprehensive_guide)
- [🚀 Production](../AutoGen_Guide/python/autogen_production_guide)
- [🎨 Diagrams](../AutoGen_Guide/python/autogen_diagrams)
- [👨‍💻 Recipes](../AutoGen_Guide/python/autogen_recipes)
- [⚡ Advanced Multi-Agent](../AutoGen_Guide/python/autogen_advanced_multi_agent)
- [📊 Observability](../AutoGen_Guide/python/autogen_observability_python)
- [🔄 AG2 Migration Guide](../AutoGen_Guide/python/autogen_to_ag2_migration_guide) **NEW**

---

### PydanticAI
**Latest:** v1.14.1

- [📂 PydanticAI Guide](../PydanticAI_Guide/)
- [📖 README](../PydanticAI_Guide/README.md)
- [🗺️ GUIDE_INDEX](../PydanticAI_Guide/GUIDE_INDEX)
- [📚 Comprehensive](../PydanticAI_Guide/pydantic_ai_comprehensive_guide)
- [🚀 Production](../PydanticAI_Guide/pydantic_ai_production_guide)
- [🎨 Diagrams](../PydanticAI_Guide/pydantic_ai_diagrams)
- [👨‍💻 Recipes](../PydanticAI_Guide/pydantic_ai_recipes)
- [⚡ Advanced Error Testing](../PydanticAI_Guide/pydantic_ai_advanced_error_testing)
- [💾 Durable Execution](../PydanticAI_Guide/pydantic_ai_durable_execution) **NEW 2025**
- [📊 Graph Support](../PydanticAI_Guide/pydantic_ai_graph_support) **NEW 2025**
- [🔗 Integrations 2025](../PydanticAI_Guide/pydantic_ai_integrations_2025) **NEW**
- [✅ Evals 2025](../PydanticAI_Guide/pydantic_ai_evals_2025) **NEW**

---

### Haystack
**Latest:** v2.19.0

- [📂 Haystack Guide](../Haystack_Guide/)
- [📖 README](../Haystack_Guide/README.md)
- [🗺️ GUIDE_INDEX](../Haystack_Guide/GUIDE_INDEX)
- [📚 Comprehensive](../Haystack_Guide/haystack_comprehensive_guide)
- [🚀 Production](../Haystack_Guide/haystack_production_guide)
- [🎨 Diagrams](../Haystack_Guide/haystack_diagrams)
- [👨‍💻 Recipes](../Haystack_Guide/haystack_recipes)
- [⚡ Advanced Agents](../Haystack_Guide/haystack_advanced_agents_python)
- [📊 Observability](../Haystack_Guide/haystack_observability_python)
- [🌊 Streaming Server](../Haystack_Guide/haystack_streaming_server_python)
- [👥 Multi-Agent Guide](../Haystack_Guide/haystack_multi_agent_guide) **NEW 2025**

---

### SmolAgents
**Latest:** v1.22.0

- [📂 SmolAgents Guide](../SmolAgents_Guide/)
- [📖 README](../SmolAgents_Guide/README.md)
- [📚 Comprehensive](../SmolAgents_Guide/smolagents_comprehensive_guide)
- [🚀 Production](../SmolAgents_Guide/smolagents_production_guide)
- [🎨 Diagrams](../SmolAgents_Guide/smolagents_diagrams)
- [👨‍💻 Recipes](../SmolAgents_Guide/smolagents_recipes)

---

### Amazon Bedrock Agents
**Latest:** GA (March 10, 2025 - Multi-Agent Collaboration)

- [📂 Bedrock Guide](../Amazon_Bedrock_Agents_Guide/)
- [📖 README](../Amazon_Bedrock_Agents_Guide/README.md)
- [📚 Comprehensive](../Amazon_Bedrock_Agents_Guide/bedrock_agents_comprehensive_guide)
- [🚀 Production](../Amazon_Bedrock_Agents_Guide/bedrock_agents_production_guide)
- [🎨 Diagrams](../Amazon_Bedrock_Agents_Guide/bedrock_agents_diagrams)
- [👨‍💻 Recipes](../Amazon_Bedrock_Agents_Guide/bedrock_agents_recipes)
- [⚡ Advanced Multi-Agent](../Amazon_Bedrock_Agents_Guide/bedrock_agents_advanced_multi_agent_python)
- [🔧 Error Handling](../Amazon_Bedrock_Agents_Guide/bedrock_agents_error_handling_python)
- [🔧 Middleware](../Amazon_Bedrock_Agents_Guide/bedrock_agents_middleware_python)
- [📊 Observability](../Amazon_Bedrock_Agents_Guide/bedrock_agents_observability_python)
- [🔐 Security & IAM](../Amazon_Bedrock_Agents_Guide/bedrock_security_iam_examples)
- [🎯 AgentCore Guide](../Amazon_Bedrock_Agents_Guide/bedrock_agentcore_comprehensive_guide) **NEW 2025**
- [🔗 Strands SDK Guide](../Amazon_Bedrock_Agents_Guide/bedrock_strands_sdk_guide) **NEW 2025**
- [🔗 A2A Protocol Guide](../Amazon_Bedrock_Agents_Guide/bedrock_a2a_protocol_guide) **NEW 2025**
- [📋 March 2025 Updates](../Amazon_Bedrock_Agents_Guide/MARCH_2025_UPDATES_SUMMARY) **NEW**

---

### Mistral Agents API
**Latest:** v1.9.11 (May 27, 2025 Launch)

- [📂 Mistral Guide](../Mistral_Agents_API_Guide/)
- [📖 README](../Mistral_Agents_API_Guide/README.md)
- [🗺️ GUIDE_INDEX](../Mistral_Agents_API_Guide/GUIDE_INDEX)
- [📚 Comprehensive/Diagrams](../Mistral_Agents_API_Guide/mistral_agents_api_comprehensive_guide)
- [🚀 Production](../Mistral_Agents_API_Guide/mistral_agents_api_production_guide)
- [👨‍💻 Recipes](../Mistral_Agents_API_Guide/mistral_agents_api_recipes)
- [⚡ Advanced Python](../Mistral_Agents_API_Guide/mistral_agents_api_advanced_python)
- [🌊 Streaming Server](../Mistral_Agents_API_Guide/mistral_streaming_server_fastapi)
- [🔌 Connectors Guide](../Mistral_Agents_API_Guide/mistral_agents_api_connectors_guide) **NEW 2025**
- [🎭 Orchestration Guide](../Mistral_Agents_API_Guide/mistral_agents_api_orchestration_guide) **NEW 2025**
- [🔗 MCP Guide](../Mistral_Agents_API_Guide/mistral_agents_api_mcp_guide) **NEW 2025**
- [📋 May 2025 Launch Updates](../Mistral_Agents_API_Guide/MAY_2025_LAUNCH_UPDATES) **NEW**

---

## 📊 Quick Reference

### By 2025 Features

**Rebranded Frameworks:**
- [Anthropic Claude SDK](../Anthropic_Claude_Agent_SDK_Guide/) (from Code SDK)
- [AutoGen → AG2](../AutoGen_Guide/python/)
- [OpenAI Agents SDK](../OpenAI_Agents_SDK_Guides/) (Swarm replacement)

**New Language Support:**
- [Google ADK - Go](../Google_ADK_Guide/go/)
- [LlamaIndex - TypeScript](../LlamaIndex_Guide/typescript/)

**Major Feature Releases:**
- [LangGraph v1.0.3](../LangGraph_Guide/python/) - Caching, hooks, memory
- [PydanticAI](../PydanticAI_Guide/) - Durable execution, graphs
- [CrewAI](../CrewAI_Guide/) - Flows
- [Amazon Bedrock](../Amazon_Bedrock_Agents_Guide/) - Multi-agent collaboration, AgentCore
- [Mistral](../Mistral_Agents_API_Guide/) - May 2025 launch with connectors

---

## 🔗 Navigation

- [← Back to Home](../)
- [Framework Comparison →](./frameworks)
- [Quick Start →](./quick-start)

---

*Last Updated: November 17, 2025*
*All links verified and updated for GitHub Pages*
